...
# Initialize control info - Set all FE-related controls to FALSE
for(d in 1:length(ctd1_datasets)) {
  control_info[[d]] <- list(
    game_situation = TRUE,
    coach = TRUE,
    ...
    team_fe = FALSE,  # Changed from TRUE to FALSE
    coach_fe = FALSE
  )
}
```
...
analyze_yardline <- function(dt, yardline_var) {
  ...
  "Opp. 9-1" = "91_100"
    ...
    # Remove season/team FE variables before they enter the model
    season_team_cols <- grep("^year\\d{4}_team_", names(dt_filtered), value = TRUE)
    if (length(season_team_cols) > 0) dt_filtered[, (season_team_cols) := NULL]
  ...
  for (yl in 1:length(yardline_ranges)) {
    ...
    if (!is.null(result)) {
      ...
      if (!is.null(result$controls)) {
        ...
        # Ensure team_fe is always FALSE
        result$controls$team_fe <- FALSE
...
# Verify the structure of control_info and ensure team_fe is always FALSE
for (d in 1:length(dataset_names)) {
  # print(paste("Control info for", dataset_names[d]))
  if (!is.null(control_info[[d]])) {
    # Force team_fe to FALSE regardless of what was detected
    control_info[[d]]$team_fe <- FALSE
    
    # Make sure all needed fields are present
    needed_fields <- c("game_situation", "coach", "team_stats", 
                     "player_vars", "player_presence", "team_fe", "coach_fe")
    for (field in needed_fields) {
      if (!(field %in% names(control_info[[d]]))) {
        # Default to FALSE for all except player_presence (TRUE)
        control_info[[d]][[field]] <- if(field == "player_presence") FALSE else FALSE
      }
    }
  } else {
    # Create empty control info with team_fe set to FALSE
    control_info[[d]] <- list(
      game_situation = TRUE,
      coach = TRUE,
      team_stats = TRUE,
...
create_dataset_table <- function(dataset_index) {
  ...
  row_labels <- c(
    ...
    "Player Variables",
    "Player Presence",
    "Season FE",  # Changed from "Season/Team FE" to "Season FE"
    "Coach FE",
    "Sample Size"
  )
  ...
  if (length(missing_controls) > 0) {
    for (ctrl in missing_controls) {
      # Default missing controls to FALSE
      dataset_controls[[ctrl]] <- FALSE
    }
  }
  
  # Ensure team_fe is FALSE
  dataset_controls$team_fe <- FALSE
  ...
  for (yl in 1:length(yardline_ranges)) {
    ...
    if (has_valid_results) {
      ...
      df[9, yardline] <- "Yes"
      df[10, yardline] <- "Yes"
      df[11, yardline] <- "No"  # Player presence is No
      df[12, yardline] <- "No"  # Season FE is No
      df[13, yardline] <- "No"  # Coach FE is No
      
      # Sample Size
      df[14, yardline] <- format(lpm_results["n"], big.mark = ",")
    } else {
  ...
  kable(df,
    ...
    pack_rows("Logit Model", 5, 6, label_row_css = "border-top: 1px solid;",
              latex_gap_space = "0.3em") %>%
    pack_rows("Controls", 7, 14, label_row_css = "border-top: 1px solid;",
              latex_gap_space = "0.3em") %>%
    footnote(
      general = c(
