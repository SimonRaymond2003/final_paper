...
control_categories <- c("Game Situation", "Coach Variables", "Team Stats", "Season FE", "Coach FE", "GIMR", "Player Presence")
...
create_control_variables <- function(is_offense) {
  ...
  for (col in model_cols) {
    ...
    if (is_offense) {
      # For offensive models: control defensive players (Yes) but not offensive players (No)
      control_vals <- c(control_vals, "Yes", "No")
    } else {
      # For defensive models: control offensive players (Yes) but not defensive players (No)
      control_vals <- c(control_vals, "Yes", "No")
    }
