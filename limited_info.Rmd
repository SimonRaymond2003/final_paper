...
# Optional: Compare regular vs starter missing data
if (length(rb_player_cols) > 0 && length(starter_rb_player_cols) > 0) {
  cat("Comparison of regular vs starter missing data:\n")
  cat("  - Regular player 4 missing data rows:", nrow(rb_player_cols[["player_4"]]), "\n")
  cat("  - Starter player 4 missing data rows:", nrow(starter_rb_player_cols[["starter_player_4"]]), "\n")
  
  # Check if they're missing in the same plays/games
  if (nrow(rb_player_cols[["player_4"]]) > 0 && nrow(starter_rb_player_cols[["starter_player_4"]]) > 0) {
    common_missing_ids <- intersect(
      rb_player_cols[["player_4"]]$my_id,
      starter_rb_player_cols[["starter_player_4"]]$my_id
    )
    cat("  - Plays/games missing in both datasets:", length(common_missing_ids), "\n")
  }
}
```
