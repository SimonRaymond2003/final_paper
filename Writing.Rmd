...
Predict attempt probability using Random Forest (RF):
\begin{equation}
z_i = \text{RF}(\mathbf{X}_i)
\end{equation}
...
In examining the following tables the significance of our GIMR is confirmed. This tells us that coaches in the NFL see factors that we cannot and act on them in the proper way. When a coach is more likly to attempt a fourth down conversion attempt that they are then also more likely to convert that fourth down attempt due to unseeen variables. 
