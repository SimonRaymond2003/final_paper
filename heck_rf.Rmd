...
create_year_fixed_effects <- function(data) {
  # Extract year from my_id (format: YYYY_XX_XXX_XXX_XXX)
  data[, year := as.integer(substr(my_id, 1, 4))]
  
  # Create dummy variables for each year (except reference year)
  years <- sort(unique(data$year))
  reference_year <- min(years)  # Use the earliest year as reference
  
  for (yr in years) {
    if (yr != reference_year) {
      var_name <- paste0("year", yr)
      data[, (var_name) := as.integer(year == yr)]
    }
  }
...
# Remove team fixed effects (any column starting with "year" and containing "team")
team_fe_cols <- grep("^year.*_team_", names(select_data), value = TRUE)
if (length(team_fe_cols) > 0) {
  select_data[, (team_fe_cols) := NULL]
}

# Create year fixed effects from my_id in select data
select_data <- create_year_fixed_effects(select_data)

# Remove team fixed effects from outcome data
team_fe_cols_outcome <- grep("^year.*_team_", names(outcome_data), value = TRUE)
if (length(team_fe_cols_outcome) > 0) {
  outcome_data[, (team_fe_cols_outcome) := NULL]
}

# Create year fixed effects from my_id in outcome data 
outcome_data <- create_year_fixed_effects(outcome_data)
  ...
  #GIMR <- z/(1-z)
  # make gimr random noise between -1 and 1
  #GIMR <- runif(nrow(rf_data), -1, 1)
...
for (dt in list(outcome_data_player, outcome_data_starter)) {
  ...
  # If 2017 is your reference year
if ("year2017" %in% names(dt)) dt[, year2017 := NULL]
  ...
  # Create patterns for present columns
  present_pattern <- paste0("(", paste(paste0(positions_to_keep, "_present$"), collapse = "|"), ")")
  
  # Create patterns for missing_information columns
  missing_info_pattern <- paste0("(", paste(paste0(positions_to_keep, "_missing_information$"), collapse = "|"), ")")
  
  # Find all present columns
  present_cols <- grep("_present$", names(dt), value = TRUE)
  
  # Find all missing_information columns
  missing_info_cols <- grep("_missing_information$", names(dt), value = TRUE)
  
  # Remove columns with zero variance (all 1s or all 0s)
  all_cols <- names(dt)
  for (col in all_cols) {
    # Check if the column exists (might have been removed in a previous step)
    if (col %in% names(dt)) {
      # If column is numeric and has zero variance
      if (is.numeric(dt[[col]]) && var(dt[[col]], na.rm = TRUE) == 0) {
        dt[, (col) := NULL]
        # Also remove from present_cols and missing_info_cols if they exist there
        present_cols <- setdiff(present_cols, col)
        missing_info_cols <- setdiff(missing_info_cols, col)
      }
    }
  }
  
  # Find columns to keep
  present_cols_to_keep <- grep(present_pattern, present_cols, value = TRUE)
  missing_info_cols_to_keep <- grep(missing_info_pattern, missing_info_cols, value = TRUE)
  
  # Find columns to remove
  present_cols_to_remove <- setdiff(present_cols, present_cols_to_keep)
  missing_info_cols_to_remove <- setdiff(missing_info_cols, missing_info_cols_to_keep)
  
  # Remove unwanted columns
  if (length(present_cols_to_remove) > 0) {
    dt[, (present_cols_to_remove) := NULL]
  }
  
  if (length(missing_info_cols_to_remove) > 0) {
    dt[, (missing_info_cols_to_remove) := NULL]
  }
  
  # For outcome_data_starter, duplicate non-starter columns with starter_ prefix
  if (identical(dt, outcome_data_starter)) {
    # Handle present columns
    ...
    # Handle missing_information columns
    missing_info_cols_no_prefix <- grep("_missing_information$", names(dt), value = TRUE)
    missing_info_cols_no_prefix <- missing_info_cols_no_prefix[!grepl("^starter_", missing_info_cols_no_prefix)]
    
    # Add duplicates with starter_ prefix for both types
    for (col in c(present_cols_no_prefix, missing_info_cols_no_prefix)) {
...
select_limited_info_starters <- fread("select_limited_info_starters.csv")
#drop my id
select_limited_info_starters[, my_id := NULL]
#cbind it to select data
select_data <- cbind(select_data, select_limited_info_starters)
...
for (i in 1:length(datasets)) {
  ...
  #glimpse(select_data)
  select_data[, DeMeco_Ryans := NULL]

  
  # Identify coach columns in select_data
coach_cols <- names(select_data)[grep("^[A-Z][a-z]+_[A-Z][A-Za-z']+", names(select_data))]

if (length(coach_cols) > 0) {
  # Create a data.table with just my_id and coach columns
  coach_dt <- select_data[, c("my_id", coach_cols), with = FALSE]
  
  # Merge coach columns into outcome_data based on my_id
  outcome_data <- merge(outcome_data, coach_dt, by = "my_id", all.x = TRUE)
  
  cat("Added", length(coach_cols), "coach columns to outcome data\n")
}
...
```{r create_matrices}
# More cautious function with separate conditions for each model type
get_model_results <- function(model) {
  ...
  # For Probit models
  if (inherits(model, "glm") && model$family$link == "probit") {
    mfx_result <- tryCatch({
      # Calculate marginal effects using probitmfx
      probitmfx(formula = formula(model), data = model.frame(model), robust = TRUE)
    }, error = function(e) {
    ...
    if (!is.null(mfx_result)) {
      # Extract the marginal effects matrix
      mfx_mat <- mfx_result$mfxest
      
      # Debug logging to verify we have the correct data structure
      cat("Column names in probit mfx_mat:", colnames(mfx_mat), "\n")
      #print out the GIMR calue ones in cat if present 
      if ("GIMR" %in% rownames(mfx_mat)) {
        cat("GIMR value in probit mfx_mat:", mfx_mat["GIMR", "dF/dx"], "\n")
      }
  ...
  # For Logit models
  if (inherits(model, "glm") && model$family$link == "logit") {
    mfx_result <- tryCatch({
      # Calculate marginal effects using logitmfx
      logitmfx(formula = formula(model), data = model.frame(model), robust = TRUE)
    }, error = function(e) {
    ...
    if (!is.null(mfx_result)) {
      ...
      # Debug logging to verify we have the correct data structure
      cat("Column names in logit mfx_mat:", colnames(mfx_mat), "\n")
  ...
  # If we get here, the model type wasn't recognized
  cat("Warning: Unrecognized model type for", deparse(substitute(model)), "\n")
  return(NULL)
}
...
for (dataset_name in names(all_outcome_models)) {
  ...
  # Get results for each model using get_model_results without passing the data
  starter_ols_results <- get_model_results(models$ols_starter_model)
  starter_probit_results <- get_model_results(models$probit_starter_model)
  starter_logit_results <- get_model_results(models$logit_starter_model)
  
  player_ols_results <- get_model_results(models$ols_model_player)
  player_probit_results <- get_model_results(models$probit_model_player)
  player_logit_results <- get_model_results(models$logit_model_player)
  
  attempt_ols_results <- get_model_results(models$ols_attempts_model)
  attempt_probit_results <- get_model_results(models$probit_attempts_model)
  attempt_logit_results <- get_model_results(models$logit_attempts_model)
  
 
  # Collect all variable names from all models
  all_vars <- unique(unlist(lapply(list(
    starter_ols_results, starter_probit_results, starter_logit_results,
    player_ols_results, player_probit_results, player_logit_results,
    attempt_ols_results, attempt_probit_results, attempt_logit_results
  ), function(x) if (!is.null(x)) names(x$ME) else NULL)))
  ...
  # Initialize lists to store results for each model type
  results_list <- list(
    starter_ols = starter_ols_results,
    starter_probit = starter_probit_results, 
    starter_logit = starter_logit_results,
    player_ols = player_ols_results,
    player_probit = player_probit_results,
    player_logit = player_logit_results,
    attempt_ols = attempt_ols_results,
    attempt_probit = attempt_probit_results,
    attempt_logit = attempt_logit_results
  )
  ########## ITS BROKEN BY HERE.################
  ...
  for (var in base_vars) {
    ...
    if (!is.null(results_list$player_ols) && var %in% names(results_list$player_ols$ME)) {
      tval_matrix[var, 4] <- results_list$player_ols$tval[var]
      mfx_matrix[var, 4] <- results_list$player_ols$ME[var]
      se_matrix[var, 4] <- results_list$player_ols$SE[var]
    }
    if (!is.null(results_list$player_probit) && var %in% names(results_list$player_probit$ME)) {
      tval_matrix[var, 5] <- results_list$player_probit$tval[var]
      mfx_matrix[var, 5] <- results_list$player_probit$ME[var]
      se_matrix[var, 5] <- results_list$player_probit$SE[var]
    }
    if (!is.null(results_list$player_logit) && var %in% names(results_list$player_logit$ME)) {
      tval_matrix[var, 6] <- results_list$player_logit$tval[var]
      mfx_matrix[var, 6] <- results_list$player_logit$ME[var]
      se_matrix[var, 6] <- results_list$player_logit$SE[var]
    }
    ...
    if (!is.null(starter_var)) {
      if (!is.null(results_list$starter_ols) && starter_var %in% names(results_list$starter_ols$ME)) {
        tval_matrix[var, 1] <- results_list$starter_ols$tval[starter_var]
        mfx_matrix[var, 1] <- results_list$starter_ols$ME[starter_var]
        se_matrix[var, 1] <- results_list$starter_ols$SE[starter_var]
      }
      if (!is.null(results_list$starter_probit) && starter_var %in% names(results_list$starter_probit$ME)) {
        tval_matrix[var, 2] <- results_list$starter_probit$tval[starter_var]
        mfx_matrix[var, 2] <- results_list$starter_probit$ME[starter_var]
        se_matrix[var, 2] <- results_list$starter_probit$SE[starter_var]
      }
      if (!is.null(results_list$starter_logit) && starter_var %in% names(results_list$starter_logit$ME)) {
        tval_matrix[var, 3] <- results_list$starter_logit$tval[starter_var]
        mfx_matrix[var, 3] <- results_list$starter_logit$ME[starter_var]
        se_matrix[var, 3] <- results_list$starter_logit$SE[starter_var]
      }
    }
    # Case 2: It's a regular variable (no starter_ prefix needed)
    else {
      if (!is.null(results_list$starter_ols) && var %in% names(results_list$starter_ols$ME)) {
        tval_matrix[var, 1] <- results_list$starter_ols$tval[var]
        mfx_matrix[var, 1] <- results_list$starter_ols$ME[var]
        se_matrix[var, 1] <- results_list$starter_ols$SE[var]
      }
      if (!is.null(results_list$starter_probit) && var %in% names(results_list$starter_probit$ME)) {
        tval_matrix[var, 2] <- results_list$starter_probit$tval[var]
        mfx_matrix[var, 2] <- results_list$starter_probit$ME[var]
        se_matrix[var, 2] <- results_list$starter_probit$SE[var]
      }
      if (!is.null(results_list$starter_logit) && var %in% names(results_list$starter_logit$ME)) {
        tval_matrix[var, 3] <- results_list$starter_logit$tval[var]
        mfx_matrix[var, 3] <- results_list$starter_logit$ME[var]
        se_matrix[var, 3] <- results_list$starter_logit$SE[var]
      }
    }
    
    # For attempt models (columns 7-9)
    if (!is.null(results_list$attempt_ols) && var %in% names(results_list$attempt_ols$ME)) {
      tval_matrix[var, 7] <- results_list$attempt_ols$tval[var]
      mfx_matrix[var, 7] <- results_list$attempt_ols$ME[var]
      se_matrix[var, 7] <- results_list$attempt_ols$SE[var]
    }
    if (!is.null(results_list$attempt_probit) && var %in% names(results_list$attempt_probit$ME)) {
      tval_matrix[var, 8] <- results_list$attempt_probit$tval[var]
      mfx_matrix[var, 8] <- results_list$attempt_probit$ME[var]
      se_matrix[var, 8] <- results_list$attempt_probit$SE[var]
    }
    if (!is.null(results_list$attempt_logit) && var %in% names(results_list$attempt_logit$ME)) {
      tval_matrix[var, 9] <- results_list$attempt_logit$tval[var]
      mfx_matrix[var, 9] <- results_list$attempt_logit$ME[var]
      se_matrix[var, 9] <- results_list$attempt_logit$SE[var]
    }
    
    # Also check for starter_ version in attempt models
    if (!is.null(starter_var)) {
      if (!is.null(results_list$attempt_ols) && starter_var %in% names(results_list$attempt_ols$ME)) {
        tval_matrix[var, 7] <- results_list$attempt_ols$tval[starter_var]
        mfx_matrix[var, 7] <- results_list$attempt_ols$ME[starter_var]
        se_matrix[var, 7] <- results_list$attempt_ols$SE[starter_var]
      }
      if (!is.null(results_list$attempt_probit) && starter_var %in% names(results_list$attempt_probit$ME)) {
        tval_matrix[var, 8] <- results_list$attempt_probit$tval[starter_var]
        mfx_matrix[var, 8] <- results_list$attempt_probit$ME[starter_var]
        se_matrix[var, 8] <- results_list$attempt_probit$SE[starter_var]
      }
      if (!is.null(results_list$attempt_logit) && starter_var %in% names(results_list$attempt_logit$ME)) {
        tval_matrix[var, 9] <- results_list$attempt_logit$tval[starter_var]
        mfx_matrix[var, 9] <- results_list$attempt_logit$ME[starter_var]
        se_matrix[var, 9] <- results_list$attempt_logit$SE[starter_var]
      }
    }
  }
