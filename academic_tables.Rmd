...
# Function to rename variables to avoid pattern matching with position prefixes
rename_variables <- function(matrix) {
  ...
  for (i in 1:length(all_row_names)) {
    ...
    # Handle "missing_information" variables
    else if (grepl("missing_information", row_name, ignore.case = TRUE)) {
...
off_grades_tval <- rename_variables(off_grades_tval)
off_yards_tval <- rename_variables(off_yards_tval)
off_completions_tval <- rename_variables(off_completions_tval)
def_stops_tval <- rename_variables(def_stops_tval)
def_grades_tval <- rename_variables(def_grades_tval)
def_tackles_tval <- rename_variables(def_tackles_tval)
...
for (dataset_idx in 1:length(dataset_names)) {
  ...
  for (col_name in names(table_data)[-1]) {
    ...
    # Set Coach FE to No for onfield and starter models, Yes for attempt models
    if ("Coach FE" %in% control_categories) {
      coach_idx <- which(control_categories == "Coach FE")
      if (grepl("On-Field|Starter", col_name)) {
        control_vals[coach_idx + 1] <- "No"
      } else if (grepl("Attempt", col_name)) {
        control_vals[coach_idx + 1] <- "Yes"
      }
    }
